...
function DriverSignupForm() {
  ...
  useEffect(() => {
    async function checkInvite() {
      if (!token) {
        setValidationError("This link is missing an invite code. Ask your dispatcher for the correct link.");
        setChecking(false);
        return;
      }
      try {
        const res = await fetch(`/api/driver-invite/validate?token=${encodeURIComponent(token)}`);
        const data = await res.json();
        if (!data.valid) {
          setValidationError(data.reason || "This invite link isn't valid.");
        } else {
          setEmail(data.email);
          setDriverFullName(data.driverFullName || "");
        }
      } catch {
  ...
  async function handleSubmit(e) {
    ...
    const res = await fetch("/api/driver-invite/complete", {
      method: "POST",
  ...
  return shell(
    <form onSubmit={handleSubmit}>
      <div className="flex items-center justify-center gap-1.5 mb-3 text-amber-400 text-xs font-semibold uppercase tracking-wide">
        <ShieldCheck size={14} />
        Private driver sign-up
      </div>
      <h1 className="text-2xl font-bold text-white mb-1 text-center">
        {driverFullName ? `Welcome, ${driverFullName}` : "Create your account"}
      </h1>
      <p className="text-gray-300 text-sm mb-6 text-center">
        This link was sent to you directly by your dispatcher. Set a password to finish setting up your own
        driver account.
      </p>
